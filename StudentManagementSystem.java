...
public class StudentManagementSystem {
    ...
    public static void findAllStudent(ArrayList<Student> array) {
        ...
        //Separate a character table for aesthetics, but not enough more

        //Remove data from the collection to display student information in a format supplemented by age.
        for (int i = 0; i < array.size(); i++) {

            Student s = array.get(i);
            //Since the data is stored, there is no data in the student object, so assign values using an array

            System.out.println(s.getSid() + "\t\t" + s.getName() + "\t\t" + s.getAge() + "year\t" + s.getAddress());
            //Called using the data method inside the student
        }
    ...
    public static void deleteStudent(ArrayList<Student>array){
        ...
        for(int i=0;i<array.size();i++){
            ...
            Student s = array.get(i);
            //The loop matches one data at a time until it matches the number we entered and then deletes, which is equivalent to scanning
